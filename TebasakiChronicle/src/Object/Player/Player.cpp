...
{
	bool press = INPUT::IsPressButton(VpadIndex::Pad0, K_Input::VpadButton::L1);
	bool stay = INPUT::IsStayButton(VpadIndex::Pad0, K_Input::VpadButton::L1);
	bool reave = INPUT::IsReaveButton(VpadIndex::Pad0, K_Input::VpadButton::L1);

	std::cout << press << stay << reave << std::endl;
	//撮影ボタンを離す
	if (INPUT::IsReaveButton(VpadIndex::Pad0,K_Input::VpadButton::L1))
	{
		//カメラマーカーをプレイヤーの位置に戻す
